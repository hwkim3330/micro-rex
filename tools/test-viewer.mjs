...
try{for(const [name,width,height]of[['desktop',1440,1000],['mobile',390,844]]){const page=await browser.newPage();const errors=[];page.on('pageerror',e=>errors.push(e.message));await page.setViewport({width,height});await page.goto(`http://127.0.0.1:${server.address().port}/web/`);await page.waitForFunction(()=>window.microRexLoaded===true,{timeout:30000});checks++;assert.equal(await page.$eval('#policy-model',e=>e.value),'microduck');assert.ok(await page.evaluate(()=>window.microRex.items.filter(m=>!m.name.startsWith('stock_')).every(m=>!m.visible)));checks++;await page.select('#policy-model','micro_rex');await page.addStyleTag({content:'html { scroll-behavior: auto !important; }'});
...
await page.select('#policy-model','micro_rex');await page.click('#reset');assert.ok(await page.evaluate(()=>window.microRex.items.every(m=>m.visible)&&window.microRex.jointControls.every(j=>!j.input.disabled&&Number(j.input.value)===0)));checks++;
