...
def run(path,velocity,seconds,seed):
    with tempfile.NamedTemporaryFile(suffix='.xml',mode='w') as f,contextlib.redirect_stdout(io.StringIO()):
        ...
        policy=up.PolicyInference(m,d,walking_onnx_path=str(ROOT/'.cache/policies/alpha_walking.onnx'),bam_ctrl=controller,new_cmd_obs=True,use_projected_gravity=True)
